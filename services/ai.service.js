...
/**
 * Claude دوره التحقق فقط (validator) — الكود هو من يقرر الـ setup.
 */
async function validateSetup({ symbol, direction, ltf, mtf, htf }) {
  const htfBias = htf.currentClose > htf.ema200 ? "BULLISH" : "BEARISH";
  const mtfBias = mtf.currentClose > mtf.ema50  ? "BULLISH" : "BEARISH";
...
`;
  ...
  const response = await client.messages.create({
    model:       "claude-sonnet-4-20250514",  // ← claude-opus-4-20250514 للدقة القصوى
    max_tokens:  150,
    temperature: 0.3,
    system: "You are a trading setup validator. Return only valid JSON. No markdown.",
    messages: [
      { role: "user", content: prompt }
    ],
  });

  // ← الفرق الرئيسي: response.content[0].text بدل response.choices[0].message.content
